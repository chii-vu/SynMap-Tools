...
def rescale(values, factor):
    ...
    """
    return {key: value * float(factor) for (key, value) in values.items()}
...
def run(dag_file, xhead, yhead, out, noks=True, xmax=800, ymax=800, flip=False, xonly=False, yonly=False):
    linewidth = 3
    # xmax = 800 #opts.xmax
    # ymax = 800 #opts.ymax

    xgenes, xchrs = replace_gff_parse(dag_file, first=True, noks=True)
    ygenes, ychrs = replace_gff_parse(dag_file, first=False, noks=True)
    ...
    if xonly:
        temp = {key : value for (key, value) in ychrs if key == xonly}

        if len(temp) == 1:
            xchrs = temp
        else:
            print("%s not in the list of chromosomes" % (xonly))
            sys.exit()

    if yonly:
        temp = {key : value for (key, value) in ychrs if key == yonly}

        if len(temp) == 1:
            ychrs = temp
        else:
            print("%s not in the list of chromosomes" % (yonly))
            sys.exit()
    ...
    boxes = draw_boxes(xchrs, ychrs, xend, yend, linewidth, xheader=xhead,
                        yheader=yhead)
    dots = draw_dots(dag_file, xgenes, ygenes, xchrs, ychrs, noks=noks)

    # Create new SVG
    myimage = svgfig.SVG('svg')
    myimage.append(dots)
    myimage.append(boxes)
    myimage.save(out+".tmp")

    with open(out+".tmp") as fh, open(out + '.svg','w') as fh2:
        for x in fh:
            if x[:4] == '<svg':
                x = SVG_HEADER_INFO
