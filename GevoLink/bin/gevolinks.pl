...
# hardcoded base url, changed tempdir to cmdarg
# $TEMPDIR  = $output;
$BASE_URL = "https://genomevolution.org/coge/";
...
sub generate_links {
        ...
        !( -r "$outfile.condensed" || -r "$outfile.condensed.gz" ) )
    {
          ...
          )
        {
            ...
            $seen{$fid1} = 1;
            my @names     = $names{$fid1};
            my $gevo_link = $BASE_URL . "GEvo.pl?fid1=$fid1;dsgid1=$id1";
            my $fids      = "fid=$fid1";
            my $count     = 2;
